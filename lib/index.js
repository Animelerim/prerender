...
exports = module.exports = function(options) {
  ...
  var hostname = options.hostname || process.env.SERVER_HOSTNAME || undefined;
  ...
  if (cluster.isMaster) {
    ...
    httpServer.listen(port, hostname, function() {
      var hosturl = 'http://' + (!hostname ? 'localhost' : hostname) + ':' + port;

      util.log('[ SUCCESS ] [ HTTP ] Server started on : ' + hosturl);
    });
