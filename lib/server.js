...
server.createPhantom = function() {
  ...
  util.log('[ INFO ] [PhantomJS] Starting...');
...
server.onPhantomCreate = function(phantom) {
  ...
  util.log('[ SUCCESS ] [PhantomJS] Started...');
  ...
  process.send({
    phantomjsPid: this.phantom.childProcess.pid
  });

  this.phantom.on('unexpectedExit', function(err) {
    util.log('[ ERROR ] [PhantomJS] Crashed, restarting...');
...
server.onRequest = function(req, res) {
  ...
  util.log('[ INFO ] [ PAGE ] ', '---' + ip + '---', req.prerender.url);
  ...
  req.prerender.page.run(_this.options.blockedResources || blockedResources, !!_this.options.logRequests, function(blockedResources, logRequests, resolve, reject) {
    ...
    this.onResourceRequested = function(requestData, request) {
      ...
      if (!requestData.aborted) {
        ...
        if (logRequests) {
          util.log('[ LOG ]', '+', _this.prerender.resourcesRequested.length - _this.prerender.resourcesReceived.length - _this.prerender.resourcesTimeout.length, requestData.url);
        }
      }
    };
    ...
    this.onResourceReceived = function(response) {
      ...
      if ('end' === response.stage) {
        if (response.url) {
          ...
          if (logRequests) {
            util.log('[ LOG ]', '-', _this.prerender.resourcesRequested.length - _this.prerender.resourcesReceived.length - _this.prerender.resourcesTimeout.length, response.url);
          }
        }
    ...
    this.onResourceError = function(resourceError) {
      if (resourceError.url && logRequests) {
        util.log('[ ERROR ] [ PAGE ] Failed to load : ', JSON.stringify(resourceError));
      }
    }
    ...
    this.settings.userAgent = this.settings.userAgent + ' Prerender (+https://github.com/Animelerim/prerender)';
...
server.checkIfPageIsDoneLoading = function(req, res) {
  ...
  req.prerender.page.run(function(resolve) {
    ...
    var match = _.find(req.prerender.headers, {
      name: 'Location'
    });
    if (match) {
...
server.checkIfJavascriptTimedOut = function(req, res) {
  ...
  if (!this.phantom || this.phantom.id !== req.prerender.phantomId) {
    ...
    util.log('[ PhantomJS ] Timed out. Sending request with HTML on the page');
    clearInterval(req.prerender.timeoutChecker);
    req.prerender.timeoutChecker = null;

    this.onPageEvaluate(req, res);
  } else if ((timeout && req.prerender.stage < 2) || noJsExecutionInFirstSecond) {
    util.log('[ PhantomJS ] Experiencing infinite javascript loop. Killing phantomjs...');
    clearInterval(req.prerender.timeoutChecker);
    req.prerender.timeoutChecker = null;
...
server.evaluateJavascriptOnPage = function(req, res) {
  ...
  req.prerender.page.run(function(resolve, reject) {
    ...
    util.log('[ ERROR ] [ PhantomJS ] Evaluating javascript!', err);
    setTimeout(_.bind(_this.evaluateJavascriptOnPage, _this, req, res), (req.prerender.evaluateJavascriptCheckTimeout || _this.options.evaluateJavascriptCheckTimeout || EVALUATE_JAVASCRIPT_CHECK_TIMEOUT));
  });
};
    ...
    if (req.prerender.headers && req.prerender.headers.length) {
      req.prerender.headers.forEach(function(header) {
        try {
          ...
          util.log('[ ERROR ] [ PhantomJS ] Unable to set header:', header.name);
        }
      });
    }
...
server._sendResponse = function(req, res, options) {
  ...
  var statusCode, status, message, ms, url;
  ...
  if (200 <= statusCode && statusCode < 300) {
    ...
    status = "[ ERROR ]";
    message = "Source could\'n create";
  } else {
    status = "[ SERVER ERROR ]";
    message = "Source could\'n create";
  }

  util.log(status, '[ PAGE ] ' + message + '\n| ', req.prerender.statusCode, 'in', ms + 'ms', 'for', req.prerender.url, '\n');
...
server._killPhantomJS = function() {
  ...
  function restartPhantom() {
    util.log("[ SUCCESS ] [ PhantomJS ] Terminated!");
    process.nextTick(_.bind(_this.createPhantom, _this));
  }
...
server._disposeAll = function(callback) {
  ...
  setTimeout(function() {
    ...
    if (!disposed) {
      util.log("[ INFO ] [ PhantomJS ] Unable to dispose PhantomJS. Forcing kill...");
      ...
      try {
        ...
        util.log("[ ERROR ] [ PhantomJS ] Failed to force killing : ", e);
  ...
  phridge.disposeAll().then(function() {
    ...
    util.log("[ ERROR ] [ PhantomJS ] Disposing all phantomjs instances : ", err);
  });
};
