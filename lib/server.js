  ...
  req.prerender.page.run(_this.options.blockedResources || blockedResources, !!_this.options.logRequests, function(blockedResources, logRequests, resolve, reject) {
    ...
    this.onResourceRequested = function(requestData, request) {
      ...
      if (!requestData.aborted) {
        ...
        if (logRequests) {
          console.log('[ LOG ]', new Date().toISOString(), '+', _this.prerender.resourcesRequested.length - _this.prerender.resourcesReceived.length - _this.prerender.resourcesTimeout.length, requestData.url);
        }
      }
    };
    ...
    this.onResourceReceived = function(response) {
      ...
      if ('end' === response.stage) {
        if (response.url) {
          ...
          if (logRequests) {
            console.log('[ LOG ]', new Date().toISOString(), '-', _this.prerender.resourcesRequested.length - _this.prerender.resourcesReceived.length - _this.prerender.resourcesTimeout.length, response.url);
          }
        }
    ...
    this.onResourceError = function(resourceError) {
      if (resourceError.url && logRequests) {
        console.log('[ ERROR ] [ PAGE ] Failed to load : ', JSON.stringify(resourceError));
      }
    }
